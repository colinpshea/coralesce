...
#' @description This function creates a data frame of all possible pairwise combinations of individuals and determines, for each  single locus, whether two individuals have the same allele or not. This function only returns comparisons for colonies with other colonies and not with themselves (the `classifyAllelePairs()` function does both for the genet assignment procedure). This function works on one locus (i.e., column) at a time. 
