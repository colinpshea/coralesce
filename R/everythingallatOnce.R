...
#' @export
runEverything <- function(PctMatchThreshold = NULL, PctNotNullThreshold = NULL, targetN = NULL){
  ...
  for (i in 1:length(fileList)){
    ...
    aa <- omitInvariantLoci(b1)
    bb <- kinshipCalcsNoInvar(dataset = aa, targetN = targetN)
    ...
    write.csv(bb[[1]], paste0(resultsLocation,"/","popAvgMKGD_", PctMatchThreshold,"_", PctNotNullThreshold,"_", nrow(b1), "_", paste0(fileList[[i]])), row.names = F)
    write.csv(bb[[2]], paste0(resultsLocation,"/","mnKinshipALL_", PctMatchThreshold,"_", PctNotNullThreshold,"_", nrow(b1), "_", paste0(fileList[[i]])), row.names = F)
    write.csv(bb[[3]], paste0(resultsLocation,"/","mnKinshipTargetN_", PctMatchThreshold,"_", PctNotNullThreshold,"_", targetN, "_", paste0(fileList[[i]])), row.names = F)
  }
  return(list(genetAssignments = d2, kinshipCalculations = bb))
}
