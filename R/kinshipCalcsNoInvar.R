...
return(list(PopAvgMKGD = PopAvgMKGD, MK_init = MK_init, MK_final = NULL))
  ...
  if (subset==TRUE){
    if (is.null(targetN)==TRUE) {stop(cat(message("subset = TRUE but you have not entered a value for targetN. When subset = TRUE, a valid targetN value (i.e., a value >0 and ≤ nrow(dataset)) must be entered for this function to work properly. You either meant to enter subset = FALSE or forgot to enter a value for targetN."), sep = "\n"))}
    dat1 <- omitInvariantLoci(dataset = dataset)
    dat2 <- determineAllAlleleMatchesOthers(dataset = dat1)
    dat3 <- kinshipCalcs(dataset = dat2)
