...
runGenets <- function(PctMatchThreshold = NULL, PctNotNullThreshold = NULL, getPairwiseAlleleMatches = FALSE){
  ...
  for (i in 1:length(fileList)){
    ...
    write.csv(d2, paste0(resultsLocation,"/","genetAssignment_", PctMatchThreshold,"_", PctNotNullThreshold, "_", paste0(fileList[[i]])), row.names = F)
    if (getPairwiseAlleleMatches==TRUE){write.csv(d1$pairwiseAlleleMatches, paste0(resultsLocation,"/","pairwiseAlleleMatches_", PctMatchThreshold,"_", PctNotNullThreshold, "_", paste0(fileList[[i]])), row.names = F)
    }
  }
