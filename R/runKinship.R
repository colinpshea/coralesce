...
#'
#' @description This wrapper function calculates pairwise kinship across all individuals and loci. Average kinship is calculated at the individual and population level, and both calculations exclude invariant loci. For this function to work properly, you **MUST** have folders called `Data` and `Results` in your working directory. This function will look for a genetics data file in `Data` and save results to the `Results` folder and pass the `Data` location path as the `fileloc` argument to `readGeneticData()`. If there is more than one file in `Data`, the function will cycle through each file and save results to the `Results` folder; the file name for each file in the `Results` folder will include the name of the original data file. 
