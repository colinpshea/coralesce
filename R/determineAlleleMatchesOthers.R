...
#' @description For each pairwise combination of individuals, this function classifies allele pairs at each locus as either matching or not. This function applies the classifyAllelePairs function, one column or locus at a time, by looping sequentially through all of `N` loci. This function ignores allele matches of colonies with themselves i.e., it only compares and assess allele matches at an individual's loci to those of other colonies, and is specifically used for kinship calculations using the `kinshipCalcsNoInvar()` function, which requires only comparisons of colonies with other colonies, not themselves.
#' @param dataset A data frame, where the first column named `Coral_ID` that uniquely identifies each row (colony), and the second through last column each contain allele data for a single locus.
