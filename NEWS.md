...
* Together these enable clone-corrected diversity in one line
    ...
    # eligible-pool diversity (all colonies, ramets included):
    computeKinship(raw)
...
- `runKinship()` gained an `@details` section explaining that kinship and gene
  diversity are computed over all colonies (ramets included) by design, and that
  `PopAvgGD` therefore describes the eligible-pool diversity rather than a
  clone-corrected, genet-level estimate.
- `kinshipCalcsNoInvar()` gained an `@seealso` pointer to that note.
